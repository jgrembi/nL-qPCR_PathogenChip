---
title: "Analyses conducted for Frontiers in Cellular and Infection Microbiology manuscript: High-throughput multi-parallel enteropathogen quantification via nano-liter qPCR"
author: "J Grembi"
date: "5/2020"
output: 
...
## ID Poor chips that should be removed from the analysis:
    # M1_2.1 & M1_2.2, M1_2.3, M3_4, M5_6, M7_8, M9_10, M11_12, M13_14, M15_16, M17_18, M19_20, and M21_22 are the first chips we ran at MSU.  This coincided with when we switched from sending samples in 1.5mL Epi tubes to sending samples in 96-well plates. It took us several months to determine that the problems with standard curve efficiency were related to DNA adherence to the plastic of the 96-well plates (lowest concentrations were coming up at later Ct values than expected).  From Chip M23+ we started pre-washing wells containing standard curves with BSA and this solved the problem!  This adherence is not expected to be a problem for fecal DNA samples as the abundance of DNA and other proteins is expected to be high enough that it wouldn't impact results of any individual assay.  Whereas with the standard curves, we had only 800 copies of total synthetic DNA per uL of TE buffer in the lowest concentrations of the standard curve pool.
...
## By assay evaluation
genes <- unique(df.compare$TargetName)
get_percent_agreement <- function(df, targetName) {
  n <- length(df$TAC_detect)
  n_posTAC <- sum(df$TAC_detect ==1)
  n_posWAF <- sum(df$WAF_detect == 1)
  actual_values <- df$TAC_detect 
  predicted_values <- df$WAF_detect 
  (conf_matrix <- table(predicted_values, actual_values))
  
  res <- epi.tests(conf_matrix)
  neg_pct_agreement <- paste0(round(res$rval$sp$est*100, 1), " (", round(res$rval$sp$lower*100, 1), ", ", round(res$rval$sp$upper*100, 1), ")")
  pos_pct_agreement <- ifelse(is.nan(res$rval$se$est), "NA", paste0(round(res$rval$se$est*100, 1), " (", round(res$rval$se$lower*100, 1), ", ", round(res$rval$se$upper*100, 1), ")"))
  
  overall.kappa <- epiR::epi.kappa(conf_matrix, method = "fleiss")
  overall_pct_agreement <- round(overall.kappa$prop.agree$obs*100, 1)
  kappa <- paste0(round(overall.kappa$kappa$est, 2), " (", round(overall.kappa$kappa$lower, 2),", ",round(overall.kappa$kappa$upper, 2), ")")
  result <- data.frame(targetName, n, n_posTAC, n_posWAF, neg_pct_agreement, pos_pct_agreement, overall_pct_agreement, kappa)
  return(result)
}

byAssay_pctAgreement <- map_dfr(genes, function (targetName) {
  df.new <- df.compare %>%
    filter(TargetName == targetName) 
  overall <- get_percent_agreement(df.new, targetName = targetName) %>%
    mutate(copy_range = "overall")
  high <- get_percent_agreement(df.new %>% filter(log.copyNum > 7), targetName = targetName) %>%
    mutate(copy_range = ">10^7")
  med <- get_percent_agreement(df.new %>% filter(log.copyNum < 7 & log.copyNum > 5), targetName = targetName) %>%
    mutate(copy_range = "10^5 - 10^7")
  low <- get_percent_agreement(df.new %>% filter(log.copyNum < 5 & log.copyNum > 0), targetName = targetName) %>%
    mutate(copy_range = "<10^5")
  
  results <- rbind(overall, high, med, low)
  return(results)
}) %>%
  rename(TargetName = targetName) %>%
  filter(TargetName != "PhHV") %>%
  mutate(TargetName = factor(TargetName, levels = c("aggR","STh","STp","eltA", "bfpA","eaeA","stx1","stx2","ipaH","cdtA","tcdB","CPE","ureA","invA","tcpA","yadA","18S_Crypto", "18S_Ehist","18S_Giardia","ITS1_Ascaris","18S_Trich")),
         labelName = factor(TargetName, levels = c("aggR","STh","STp","eltA", "bfpA","eaeA","stx1","stx2","cdtA","tcdB","CPE","ureA","invA","ipaH", "tcpA","yadA","18S_Crypto", "18S_Ehist","18S_Giardia","ITS1_Ascaris","18S_Trich"), labels = c("EAEC (aggR)", "ST-ETEC (STh)", "ST-ETEC (STp)", "LT-ETEC (eltA)", "EPEC (bfpA)", "EPEC (eaeA)", "STEC (stx1)", "STEC (stx2)", "Campylobacter jejuni/coli (cdtA)", "Clostridium difficile (tcdB)", "Clostridium perfringens (CPE)", "Helicobacter pylori (ureA)", "Salmonella enterica (invA)", "Shigella/EIEC (ipaH)", "Vibrio cholerae (tcpA)", "Yersinia enterocolitica (yadA)", "Cryptosporidium (18S)", "Entamoeba histolytica (18S)", "Giardia (18S)", "Ascaris lumbricoides (ITS1)", "Trichuris trichiura (18S)"))) %>%
  arrange(labelName) %>%
  select(labelName, n:n_posWAF, overall_pct_agreement, kappa, neg_pct_agreement, pos_pct_agreement, copy_range)
  
table_s4 <- byAssay_pctAgreement %>%
  filter(copy_range == "overall") %>%
  select(-copy_range) %>%
  left_join(byAssay_pctAgreement %>% 
              filter(copy_range != "overall") %>%
              pivot_wider(id_cols = c(labelName), names_from = copy_range, values_from = c(n, pos_pct_agreement)) %>%
              select(labelName, ends_with(">10^7"), ends_with("10^5 - 10^7"), ends_with("<10^5"))) 
  
write.csv(table_s4, "/Users/JGrembi/Dropbox/01_PathogenChip/Manuscript/Frontiers Submission/tableS4.csv")
...
write.csv(table2, "/Users/JGrembi/Dropbox/01_PathogenChip/Manuscript/Frontiers Submission/table2.csv")
# 
##Make table
options(knitr.kable.NA = '')
# table2 %>%
