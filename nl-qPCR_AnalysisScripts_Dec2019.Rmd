...
#Load smoothing functions
source(paste0(path2MainDir,"SmoothingFunctions.R"))
...
##Inflammation & diarrhea against pathogen data
```{r}


df.outcomes <- df.compare.diar %>%
  mutate(Organism = factor(Organism),
         WAF_Ct = ifelse(WAF_meanCt == 99, 40, WAF_meanCt)) %>%
  filter(Result != "00") %>% 
  left_join(df.cov.full %>% select(childid, st_aged2, neo2, mpo2, aat2,LM2)) #%>%
  # filter(diar7d == 1)
  # filter(TAC_Ct <= OR_Ct)

ggplot(df.outcomes) + 
  geom_point(aes(x = TAC_Ct, y = WAF_Ct, color = diar7d)) + 
  facet_wrap(~Organism, scales = "free")

byChild <- df.compare.diar %>%
  mutate(Organism = factor(Organism),
         WAF_Ct = ifelse(WAF_meanCt == 99, 40, WAF_meanCt)) %>%
  # left_join(df.cov.full %>% select(childid, st_aged2, neo2, mpo2, aat2,LM2)) %>%
  # pivot_wider(id_cols = c(childid, st_aged2:LM2, diar7d),
  #             names_from = TargetName,
  #             values_from = Result) %>%
  filter(diar7d == 1, labelName != "PhHV (gB)") %>%
  # select (-PhHV) %>%
  group_by(childid) %>%
  summarise(n11 = sum(Result == "11"),
            n01 = sum(Result == "01"),
            n10 = sum(Result == "10"))
